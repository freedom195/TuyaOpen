...
{
    if(cellular_mds_desc_get()->mds_init == NULL) {
        return OPRT_NOT_SUPPORTED;
    }
    CHECK_SIM_ID(sim_id);
    return cellular_mds_desc_get()->mds_init(sim_id);
}
...
{
    if(cellular_mds_desc_get()->get_mds_status == NULL) {
        return OPRT_NOT_SUPPORTED;
    }
    CHECK_SIM_ID(sim_id);
    return cellular_mds_desc_get()->get_mds_status(sim_id);
}
...
{
    if(cellular_mds_desc_get()->pdp_active == NULL) {
        return OPRT_NOT_SUPPORTED;
    }
    CHECK_SIM_ID(sim_id);
    OPERATE_RET ret =  cellular_mds_desc_get()->pdp_active(sim_id, apn, username, password);
    if(ret == OPRT_OK) {
...
{
    if(cellular_mds_desc_get()->adv_pdp_active == NULL) {
        return OPRT_NOT_SUPPORTED;
    }
    CHECK_SIM_ID(sim_id);
    OPERATE_RET ret =  cellular_mds_desc_get()->adv_pdp_active(sim_id, cid, pdp_type, apn, username, password);
    if(ret == OPRT_OK) {
        s_pdp_start[sim_id] = true;
    }
    return ret;
...
{
    if(cellular_mds_desc_get()->pdp_deactive == NULL) {
        return OPRT_NOT_SUPPORTED;
    }
    CHECK_SIM_ID(sim_id);
    OPERATE_RET ret = cellular_mds_desc_get()->pdp_deactive(sim_id);
    if(ret == OPRT_OK) {
...
{
    if(cellular_mds_desc_get()->adv_pdp_deactive == NULL) {
        return OPRT_NOT_SUPPORTED;
    }
    CHECK_SIM_ID(sim_id);
    OPERATE_RET ret = cellular_mds_desc_get()->adv_pdp_deactive(sim_id, cid);
    if(ret == OPRT_OK) {
        s_pdp_start[sim_id] = false;
    }
    return ret;
...
{
    if(cellular_mds_desc_get()->pdp_auto_reactive_enable == NULL) {
        return OPRT_NOT_SUPPORTED;
    }
    CHECK_SIM_ID(sim_id);
    return cellular_mds_desc_get()->pdp_auto_reactive_enable(sim_id, enable);
}
...
{
    if(cellular_mds_desc_get()->registr_mds_net_notify == NULL) {
        return OPRT_NOT_SUPPORTED;
    }
    CHECK_SIM_ID(sim_id);
    return cellular_mds_desc_get()->registr_mds_net_notify(sim_id, fun);
}
...
{
    if(cellular_mds_desc_get()->get_ip == NULL) {
        return OPRT_NOT_SUPPORTED;
    }
    CHECK_SIM_ID(sim_id);
    return cellular_mds_desc_get()->get_ip(sim_id, ip);
}
